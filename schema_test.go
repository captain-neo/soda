...
func TestOAIGenerator_GenerateJSONRequestBody(t *testing.T) {
	...
	assert.Equal(t, schema.Properties["int32"].Value.Max, math.MaxInt32)
	assert.Equal(t, schema.Properties["int32"].Value.Min, math.MinInt32)
	assert.Equal(t, schema.Properties["uint32"].Value.Max, math.MaxUint32)
	assert.Equal(t, schema.Properties["uint32"].Value.Min, 0)
}
