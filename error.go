...
func (ve ValidationError) Error() string {
	return fmt.Sprintf("validation error: field %s in %s is invalid, cause of %s", ve.Field, ve.Position, ve.Reason)
}
...
func (e SerializationMethodError) Error() string {
	return fmt.Sprintf("invalid serialization method: style=%s, explode=%v", e.Style, e.Explode)
}
